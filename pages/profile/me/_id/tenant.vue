    ...
    <div class="page-search">
      ...
      <div>
        <div class="form-group">
          ...
          <nuxt-link class="btn btn-dark btn-sm" :to="{ name: 'profile-dashboard-tenant' }">
...
export default {
  ...
  mounted() {
    ...
    if (!authId) {
      this.$router.push({ name: 'profile-dashboard-tenant' });
    }
  },
  async created() {
    ...
    async update() {
      await this.$axios.$post('profile/tenant/update/' + this.$auth.user.tenant_id, this.form,)
        .then(response => {
          ...
          this.$router.push({ name: 'profile-me-id-tenant' });
        })
        .catch(error => {
